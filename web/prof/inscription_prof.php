...
$select = mysqli_query($link, "SELECT * FROM enseignant WHERE email = '".$_REQUEST['email1']."'");
if(mysqli_num_rows($select)) {
    $result='<div class="alert alert-danger">Cette adresse email est déjà utilisé</div>';
    
    header("location: /prof/inscription.php?result=".urlencode($result));
...
$sql = "INSERT INTO enseignant (nom,prenom,password,email,status,token) VALUES (?,?,?,?,?,?)";
...
if($stmt = mysqli_prepare($link, $sql)){
    ...
    if(mysqli_stmt_execute($stmt)){
        ...
        $url = 'http://'.$_SERVER['SERVER_NAME'].'/prof/verify.php?id='.$last_id.'&token='.$token;
        ...
        <br>'.$url.'</div>';$last_id = mysqli_insert_id($link);
           ...
           // $mail->AltBody = 'This is the body in plain text for non-HTML mail clients';
            ...
            if(!$mail->send()) {
                ...
                $result='<div class="alert alert-success">felisitation vous pouvez verifier votre mail pour terminer </div>';
