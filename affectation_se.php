...
$select7 = mysqli_query($link,"select id from etudiant where classement='1' and spécialité ='se'");
...
$update = mysqli_query($link,"update etudiant set sujet_id ='$sujet_id8' where classement='1' and spécialité ='se' ");
$update3 = mysqli_query($link,"update sujet set etat ='affecté' where id ='$sujet_id8' ");

for ($i=2;$i<=$rowcount;$i++) {
  for($p=1;$p<=15;$p++) {

    $select = mysqli_query($link,"select id from etudiant where classement=$i and spécialité ='se'");
    ...
    for ($c=1;$c<$i;$c++) {
                     ...
                     $select3 = mysqli_query($link,"select sujet_id from etudiant where classement=$c and spécialité ='se'");
                     ...
                     $sujet_id3=$data3['sujet_id'];
                      ...
                      if($sujet_id2!=$sujet_id3){
                        $update2 = mysqli_query($link,"update etudiant set sujet_id ='$sujet_id2' where classement='$i' and spécialité ='se' ");
  ...
  for ($j=2;$j<=$rowcount;$j++) {
    $select10 = mysqli_query($link,"select sujet_id from etudiant where classement='$j' and spécialité ='se'");
    $data10 = mysqli_fetch_assoc($select10);
    $sujet_id10=$data10['sujet_id']; 
     if(is_null($sujet_id10)){

      $select11 = mysqli_query($link,"select id from sujet where etat='validé' and spécialité ='se' ");
...
/*       $id = array_keys($data11['id']);
      ...
      $update11 = mysqli_query($link,"update etudiant set sujet_id ='$sujet_id11' where classement='$j' and spécialité ='se' ");
